...
The dataset used for all tables, plots and analysis is retrieved from [Our World in Data](https://ourworldindata.org/coronavirus/country/chile) on **1 September 2021**, with all data up to date until **31 August 2021**. This dataset is publicly available and completely open access under the Creative Commons BY license, and is allowed to be used for analysis purpose of this blog.
...
The original dataset includes data from `r length(unique(df$location))` countries, and contains `r ncol(df)` columns and `r nrow(df)` observations in total. This blog only focus on the COVID-19 situation in Chile, and the areas of interests are only related to confirmed cases, number of tests conducted, number of death, vaccinations performed, and relevant demographics of the local population. 

### Data Cleaning

Therefore, the dataset has been filtered and selected with only relevant variables and observations from Chile, stored in a tidy format for analysis purpose of this report.
