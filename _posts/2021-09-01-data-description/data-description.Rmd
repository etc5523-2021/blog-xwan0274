...
The original dataset includes data from `r length(unique(df$location))` countries, contains `r ncol(df)` columns and `r nrow(df)` observations. This blog only focus on the COVID-19 situation in Chile, and the areas of interests are only related to confirmed cases, number of tests conducted, number of death and vaccinations performed. 

### Data Cleaning

Therefore, the dataset has been filtered and selected with only records associated with Chile and relevant variables, stored in a tidy format for analysis purpose of this report.
